...
The load context also carries one non-public, symbol-keyed capability object (read through `@bgub/fig/internal`) with two generation-guarded operations. fig-dom's `payloadDataLoader` uses `hydrate` to apply a payload stream's `data` rows through the calling store; it acts only while the load generation is authoritative and skips rows targeting the loading entry's own key (a loader cannot supersede itself with its own data row). `attributeError` attributes a rejected streamed hole to that owning generation for as long as the generation's signal is live — the signal's lifetime is its authority, so attribution can fire before the loader's own promise settles and the visible value keeps attributing through a superseding refresh's window. A live attribution lets boundary recovery call `invalidateDataError` against a fulfilled entry whose nested value failed, retiring that broken value; retired generations are ignored, and a generation that never publishes cannot make a value it doesn't own invalidatable through its errors.
