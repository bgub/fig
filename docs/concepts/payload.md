...
- `prepareAssets(assets)` is called with asset resources as soon as their rows arrive (fig-dom's `insertAssetResources` is the intended web implementation). It inserts both stream assets and title/meta document state. A returned promise gates the reveal of only the content that declared a dependency — the row named by an assets row's `for`, or the island referencing a `client` row's own assets. Gate settlement (fulfilled _or_ rejected) releases the reveal; a failed asset never blocks content.
