...
- A reference that `resolveClientReference` resolves and that carries no asset gate decodes to the resolved component itself, so its element type is identity-stable across decodes: re-decoding a surrounding payload (a refresh) updates the client component in place and its state survives. Gated or `loadClientReference`-loaded references decode to per-decode wrapper components and remount on re-decode.
