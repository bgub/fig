...
- `@bgub/fig-server` — server rendering (`renderToStream` grid, `prerender`) and the `./payload` server half (`renderToPayloadStream` and the inline frame transport); the payload format itself lives in `@bgub/fig/payload`, and the serializer builds against it through the internal entry. `escapeAttribute`/`escapeText` are exported because their contract is "consistent with fig-server's own HTML emission".
