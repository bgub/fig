...
- `@bgub/fig` — the component model: elements and the JSX runtime, components (`Fragment`, `Suspense`, `Activity`, `ErrorBoundary`, `ViewTransition`), hooks, the read verbs (`readContext`, `readPromise`, `readData`), `transition`, `isValidElement`, `lazy`, `clientReference`, asset-resource creators, and the data layer — `dataResource`, the freshness verbs, and the store implementation (which stays out of data-free bundles; see the lazy store installation protocol below). The `./payload` subpath is the browser-safe payload-format home: row/model types, codecs, the value codec, and the client decoder `decodePayloadStream` (payload.md) — browser code never imports `@bgub/fig-server` to decode. The `./server` subpath is the server-file authoring entry (`serverDataResource`); its packaging transform is `figData` in `@bgub/fig-vite`. That package is currently a private workspace preview, not part of the synchronized public release group; Fig Start consumes it in-repo while the standalone bundler contract matures.
...
- `@bgub/fig-server` — server rendering (`renderToStream` grid, `prerender`) and the `./payload` server half (`renderToPayloadStream`, the inline frame transport, and the legacy consumer pending deletion); the payload format itself lives in `@bgub/fig/payload`, and the serializer builds against it through the internal entry. `escapeAttribute`/`escapeText` are exported because their contract is "consistent with fig-server's own HTML emission".
