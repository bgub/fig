...
- Start's private default server entry imports TanStack Start Server Core directly and pairs it with a narrow renderer module. It does not import the public Start server module, which also owns Payload response rendering and its compiled application-reference manifest. The renderer module is an implementation seam shared with the public server API, not a package entrypoint.
