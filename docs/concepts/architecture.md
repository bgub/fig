...
- `@bgub/fig-reconciler` — renderer authoring: `createRenderer`/`HostConfig`, `EventPriority`/`runWithEventPriority`, the internal cooperative scheduler, and the `./devtools` + `./refresh` + `./test-utils` (`act`) subpaths.
