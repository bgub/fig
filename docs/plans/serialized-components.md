...
- Implementation notes from Phase 1:
  - Asset dependency is declared on the wire: `assets` rows gained an optional `for` (the emitting task's row id), so the decoder gates exactly that row's reveal instead of guessing positionally. Client-reference assets gate only the island (the gate lives inside the island component, not the referencing chunk).
...
- **Phases 2–5** — not started (fig-dom `payloadDataLoader` + generation-lifetime store contract; parity/perf validation; fig-start migration; targeted-refresh removal).
