...
Status: complete — all phases landed 2026-07-13; retained as the design record
...
- **Phase 4 (fig-start migration)** — landed 2026-07-13. Server routes are ordinary data resources keyed `[routeId, url]`: fig-start's payload responses are plain streams (no `PayloadBoundary` wrap, no boundary header), the client's `ServerRouteContent` store wraps a `dataResource` + `payloadDataLoader` (refresh = `refreshData`; child navigation = new URL key; back/forward = cached keys), and navigation atomicity holds the router commit on `refreshData` + tracked island-module loads + tracked stylesheet gates (a `prepareAssets` override on `payloadDataLoader`). Frame-bound streamed hydration: the initial document segment's loader `request` returns a synthetic `Response` over the inline frame stream (frames gained an `end` marker to close it), so pending holes keep filling through the same generation-guarded path a fetched payload uses; initial-document assets insert-for-dedupe but never gate (the SSR head already delivered them styled). The document's SSR-side consumer was replaced by server-side `decodePayloadStream` (new `onClientReference` observer feeds the ssr-module bootstrap; non-ssr references decode to the same placeholder template the client hydrates). Implementation notes: the island hydration-gate reveal must run in a passive effect on the content reader (external-store subscriptions attach at commit, so layout-phase reveals fire before any island subscribes and are lost), and the initial-segment "don't gate assets" flag must key off the loader's request, not the frame stream (the end frame replays synchronously at subscription time).
- **Post-plan refinement (2026-07-13):** supersession abort is deferred — a refreshed entry's previous generation keeps its signal (stale tree streams on) until the successor's value publishes; a failed refresh leaves it fully alive. Motivated by the demo repro where refreshing mid-stream surfaced "Payload decode aborted." through the page ErrorBoundary; the retirement doctrine (errors.md) now actually holds for plain consumers.
- **Phase 5 (targeted-refresh removal)** — landed 2026-07-13, gate verified (fig-start client-dom tests + demo-start/demo-payload e2e on the resource model). Deleted from `@bgub/fig-server/payload`: `createPayloadConsumer`/`PayloadConsumer`, `PayloadBoundary` + duplicate-id dev check, `PAYLOAD_BOUNDARY_HEADER`, `PayloadFetchError`, refresh id-range namespacing, the consumer decode machinery, and `renderToPayloadStream`'s `refreshBoundary` option; the `refresh`/`refresh-error` rows and `$fig:"boundary"` model left the wire format. apps/demo-payload became the standalone resource-model demo (the example at the top of this plan); the guides teach `payloadDataLoader`/`decodePayloadStream`. fig-server's payload.ts went from ~3.1k to ~1.1k lines.
