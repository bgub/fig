...
- **Native TanStack Start framework target** — the Vite adapter is shipped and delegates builds plus server-function compilation to plugin core through a private Solid compatibility target. Upstream still needs an extensible framework descriptor (package ids, route templates, compiler imports, refresh behavior) before Fig can remove those mappings and generate native Fig file routes. The runtime, store, and hydration contracts do not depend on that change.
