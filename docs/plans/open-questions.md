...
- **TanStack Start Vite adapter** — the runtime layer now owns a single Fig store across route loading and render/hydration, with a Fig-encoded document snapshot. The remaining bundler layer needs TanStack's plugin core to admit framework adapters beyond its closed React/Solid/Vue target union (or a deliberately maintained Fig plugin).
