...
- Done: **hydration-mismatch opt-out** uses React's `suppressHydrationWarning` surface for one host element level.
- Done: **data loader context removed**. Fig-data loaders receive resource args plus `{ signal }`; framework/request dependencies live outside the primitive.
