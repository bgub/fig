...
- Document resources: `assets([...], children)` attaches resources to a
  ...
  Server rendering exposes `title` and `meta` as document head output;
  stylesheets are emitted near the segments that depend on them and ordered
  deterministically by precedence and URL.
